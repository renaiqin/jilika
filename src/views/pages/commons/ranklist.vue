<template>
  <div class="m_RankList">
        ...
        <!--  -->
        <div v-if="zspm == 1" class="m_Rmianbox">
            ...
            <ul class="u_Rlist">
                ...
                <!-- v-if="localStorage.getItem('usertype')=='4'" -->
                <li v-if="usertype == '4' && myRankList.length != 0 && myRankList[0].jlkyftj.kplx == '总榜主'">
                    <div class="u_Rbox">
                        <div class="f_imgleft">
                            <img src="/static/img/zbz.png" class=""/>
                        </div>
                        <div class="f_infomid">
                            <p>{{myRankList[0].jlkyftj.xm}}</p>
                            <p class='f_grade'>{{myRankList[0].jlkyftj.bjmc}}</p>
                        </div>
                        <div class="f_right">
                        <span class="spanwarp">{{myRankList[0].jlkyftj.hks}}</span>
                        <p class="f_ShowList" v-if="myRankList[0].childrenLyb&&myRankList[0].childrenLyb.length>=1" v-show="!myRankList[0].isOpen" @click="moreList(myRankList[0])">查看{{myRankList[0].childrenLyb.length}}名并列 <i></i></p>
                        </div>
                    </div>

                    <div v-if="myRankList[0].childrenLyb&&myRankList[0].childrenLyb.length>=1" v-show="myRankList[0].isOpen" class="u_RSubList">

                        <div class="u_Rbox" v-for="(sitem,index) in myRankList[0].childrenLyb">
                            <div class="f_imgleft">
                                <img src="/static/img/zbz.png" class=""/>
                            </div>
                            <div class="f_infomid">
                        ...
                        <div class="f_CloseList" @click="closeList(myRankList[0])"></div>
                    </div>
                </li>
                ...
                <li v-for="(item,index) in cardList" v-if="item.jlkyftj.xm">
                    <div class="u_Rbox">
                        <div class="f_imgleft">
                            <img :src="item.jlkyftj.kplx == '总榜主'? '/static/img/zbz.png':item.jlkyftj.tplj?item.jlkyftj.tplj:'/static/img/test1.png'" class=""/>
                        </div>
                        <div class="f_infomid">
                            <p>{{item.jlkyftj.xm}}</p>
                        ...
                        <div class="f_right">
                            <span class="kplxText">{{item.jlkyftj.kplx}}</span>
                        <span class="spanwarp">{{item.jlkyftj.hks}}</span>
                        <p class="f_ShowList" v-if="item.childrenLyb&&item.childrenLyb.length>=1" v-show="!item.isOpen" @click="moreList(item)">查看{{item.childrenLyb.length}}名并列 <i></i></p>
                        </div>
                    </div>

                    <div v-if="item.childrenLyb&&item.childrenLyb.length>=1" v-show="item.isOpen" class="u_RSubList">
                        ...
                        <div class="u_Rbox" v-for="(sitem,index) in item.childrenLyb">
                            ...
                            <div class="f_right">
                                <span class="kplxText">{{sitem.kplx}}</span>
                                <span class="spanwarp">{{sitem.hks}}</span>
                            </div>
                        </div>
        ...
        <div v-else  class="m_Rmianbox">
            ...
            <ul class="u_GradeRankList">
                <li v-if="usertype == '4'  && myRankList.length != 0 && myRankList[0].jlkyftj">
                    <div v-if="active_type == 0"  class="card_rank no">{{myRankList[0].jlkyftj.qxpm}}</div>
                    <div v-if="active_type == 1"  class="card_rank no">{{myRankList[0].jlkyftj.njpm}}</div>
                    <div v-if="active_type == 2"  class="card_rank no">{{myRankList[0].jlkyftj.bjpm}}</div>
                    <div class="card_name">我</div>
                    <div class="card_grade">{{myRankList[0].jlkyftj.bjmc}}</div>
                    <div class="card_num">{{myRankList[0].jlkyftj.hks}}</div>

                    <div v-if="myRankList[0].childrenLyb && myRankList[0].childrenLyb.length >=1">
                        <div class="f_ShowList" v-show="!myRankList[0].isOpen" @click="moreList(myRankList[0])">查看{{myRankList[0].childrenLyb.length}}名并列 <i></i></div>
                        <div class="u_RSubList" v-show="myRankList[0].isOpen">
                            <li v-for="(sitem,index) in myRankList[0].childrenLyb">
                ...
                <li v-for="(item,index) in cardList">
                    ...
                    <div v-if="item.childrenLyb && item.childrenLyb.length >=1">
  ...
  export default {
    ...
    methods: {
        async init(){
            ...
            //获取当前学年学期
            this.xndm = localStorage.getItem("xn");
            this.xqdm = localStorage.getItem("xq");
        ...
        async getCardList(obj){
            ...
            var parmaobj = {
                ...
                nf:this.nf,
                yf:this.yf,
                //xh: localStorage.getItem("usertype")=="4" ? localStorage.getItem('xh') : null
            };
            //获取卡片列表
            ...
            // this.cardList = data;
             ...
             this.myRankList = [];
            var {data} = await this.$post(api.geLybList(),parmaobj,{headers:'application/json'});
            if(localStorage.getItem("usertype")=="4" && this.zspm != 1){
                this.cardList = data.slice(0,data.length-1);
                this.myRankList = data.slice(data.length-1,data.length);
            }else{
                this.cardList = data;
            }
        ...
        confirmtime(value){
            ...
            if(this.active_type==0){
                if(this.zspm > 1){
                  this.getCardList({lx:'qx',kplx:this.selLxValue == -1 ? 'all' : this.selLxValue})
                   console.log(this.selLxValue,6666)
                }else{
            ...
            // }else{
            //     if(this.active_type==0){
            //         this.getCardList({lx:'qx',kplx:''});
            //     }else if(this.active_type==1){
            //         this.getCardList({lx:'nj',njdm:this.selNjValue,kplx:''});
            //     }else if(this.active_type==2){
            //         this.getCardList({lx:'bj',bjdm:this.selBjValue,kplx:''});
            //     }
            // }
            this.isshow = false;
            this.ismask = false;
