<template>
  <div class="selectwarp">
    ...
    <div v-else class="cardwarp"  ref="scrollBox" @scroll="watchScroll()">
        <div class="groupwarp" :class="{'on_del':item.isOnDel}" v-for="(item,index) in cardlist" @click="cardDetail(item.jlid,item.xh)" @touchstart="cardTouchStart($event,item)" @touchmove="cardTouchMove($event,item)">
    ...
    <transition name="upinto">
        <van-picker v-if="isChangeWeek" show-toolbar title="选择时间" :columns="weekList" @cancel="onCancel"
    @confirm="onConfirm" class="f_week_picker"/>
    </transition>
        ...
        */
        ...
        async queryPageList(){
            ...
            //await 
                ...
                var res = await this.$post(api.queryPageList(),{xxdm: localStorage.getItem('xxdm'),xqdm:localStorage.getItem('xq'),xndm:localStorage.getItem('xn'),page:this.page,limit:10,beginTime:this.beginTime,endTime:this.endTime,xh:localStorage.getItem("xh")},{headers:'application/json'});
