...
export default {
  ...
  methods: {
     ...
     */
    async findqx(key) {
      await this.qxlistFun();
      let that = this;
        //alert(JSON.stringify(that.qxlist));
     // alert(JSON.stringify(this.qxlist))
     let index =_.findIndex(that.qxlist, function(item) { return item.ydgndm == key });
        //  that.qxlist.findIndex((value, index, arr) => {
        //   slert(value);alert(index);alert(arr);
    ...
    */
     ...
     */
    async getXxdtList(){
      this.$fetch(api.getXxdtList(),{xxdm:localStorage.getItem('xxdm'),userid: localStorage.getItem('xh'),usertype:localStorage.getItem('usertype'),classCode:''}).then(res=>{
    ...
    async getXxdtNum(){
      ...
      this.$fetch(api.getXxdtNum(),{xxdm:localStorage.getItem('xxdm'),userid: localStorage.getItem("xh"),usertype:usertype}).then(res=>{
  ...
  mounted() {
    ...
    Promise.all([this.getTopKpxxList(),this.getMyKpxxList(),this.getXxdtList(),this.getXxdtNum()]);
    //alert(7766)
  }
};
</script>
