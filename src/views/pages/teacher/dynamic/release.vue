  ...
  methods: {
    ...
    async init() {
      ...
      if (
        ...
        //console.log(this.kplx,66666)
        //.log(this.kplx,55555555555)
      }
    },
    /**
     * 说明:格式化类型列表
    ...
    formatCardType(data) {
      ...
      let classList = this.$store.getters.getSelectAllClass;
      if(studentList.length == 0 && classList.length ==0){//如果没有所选学生 则展示该老师有权限班级的卡片
        ...
        for (var i = 0; i < data.length; i++) {
          typearr.push({
            ...
            sfsj: data[i].sfsj,
            kpsl: data[i].kpsl,
            gradeList:data[i].gradeList,
            index:i
          });
          
        }
      }else{//当选择学生时，则展示所选学生的卡片
      
        data.forEach((item,index)=>{
          for(var i=0;i<studentList.length;i++){
            if(item.gradeList.includes(studentList[i].njdm)){
              typearr.push({
                text: item.kpmc,
                value: item.kpid,
                tplj: item.tplj,
          ...
          for(var i=0;i<classList.length;i++){
            if(item.gradeList.includes(classList[i].njdm)){
              typearr.push({
                text: item.kpmc,
                value: item.kpid,
                tplj: item.tplj,
      ...
      subarr = this.formatSubCardType(typearr[0].index);
      resarr.push(
        { className: "cardtype", values: typearr },
        { className: "cardsubtype", values: subarr }
      );
      //console.log(resarr,787878787);
      return resarr;
    },
    /**
     * 说明:格式化子类型列表，获取子类型列表
    ...
    formatSubCardType(index) {
      ...
      // console.log(index,5555555555555)
      var subarr = [];
      //console.log(data[index],33333333333)
    ...
    async formatAllStudentList() {
      ...
      //.log(studentdata,classdata,gradedata,77777777)
      let classParam = [];
      let gradeParam = [];
      //生成获取班级学生需要的班级参数列表
    ...
    async onConfirm(value) {
      ...
      // console.log(value)
      this.$store.dispatch("setSelectCard", value);
    ...
    async releaseDynamic() {
      ...
      if(typeof(this.attachmentList) != 'undefined'){
        var imgpathlist = [];
...
.m_botRelease {
  padding: px2rem(50px) px2rem(55px);
  background: #fff;
